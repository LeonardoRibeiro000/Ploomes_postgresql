...
# Configuração da API
url = "https://api2.ploomes.com/Deals@Pipelines"
...
def sanitize_column_name(name):
    """Normaliza e sanitiza nomes de colunas para o PostgreSQL"""
...
def map_pandas_to_postgres(dtype):
    """Mapeia tipos do pandas para tipos do PostgreSQL"""
...
def create_table_from_dataframe(df, table_name):
    ...
    try:
        ...
        col_definitions = ", ".join(
            f"{col} {map_pandas_to_postgres(df[col])}" 
...
def insert_new_data(df, table_name):
    """Insere novos dados no PostgreSQL"""
    conn = None
    cursor = None
    try:
        conn = get_pg_connection()
        ...
        # Sanitiza colunas antes de filtrar
        df.columns = [sanitize_column_name(col) for col in df.columns]
...
if __name__ == "__main__":
    ...
    if dados:
        ...
        df = drop_column(df, 'cpqproductsusemodal')  # Nome já sanitizado
        table_name = "dfunil"
